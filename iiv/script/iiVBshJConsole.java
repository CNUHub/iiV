...
/**
    ...
    Modified by: Joel Lee <jtlee@james.psych.umn.edu>
      renamed to fit in iiV package structure. Use should be temporary until they fix paste lock up problems
      with bsh.util.JConsole.
...
*/
public class iiVBshJConsole extends JScrollPane
    implements GUIConsoleInterface, Runnable, KeyListener,
    MouseListener, ActionListener, PropertyChangeListener 
{
    private static final long serialVersionUID = -8264759245731158725L;
    ...
    public Reader getIn() { return new InputStreamReader(getInputStream()); }
    ...
    public iiVBshJConsole() {
        this(null, null);
    }

    public iiVBshJConsole( InputStream cin, OutputStream cout )  
    {
        super();
        ...
        // create popup    menu
        menu = new JPopupMenu("iiVBshJConsole    Menu");
        ...
        outPipe    = cout;
        if ( outPipe ==    null ) {
            outPipe    = new NicePipedOutputStream();
            try {
                // handle ephemeral RMI threads for long-running commands
                ...
                print("Console internal error: "+e.getMessage(), Color.red);
            }
        }
        ...
        if ( inPipe == null ) {
            ...
            try {
                ...
                print("Console internal error: "+e.getMessage());
            }
        }
    ...
    private String getCmd() {
        ...
        try {
            ...
            // should not happen
            System.out.println("Internal iiVBshJConsole Error: "+e);
        }
        return s;
    }
    ...
    private void acceptLine( String line ) 
    {
        ...
        if (outPipe == null )
            print("Console internal error: no output pipe", Color.red);
        else {
            byte[] bytes = line.getBytes();
            handler.addBytes( bytes );
        }
    }
    ...
    /**
        ...
        This is used by the iiVBshJConsole internal read thread to allow writers
    ...
    */
    ...
    static class NicePipedOutputStream extends PipedOutputStream {
        protected PipedInputStream _sink;


        public NicePipedOutputStream( PipedInputStream s ) throws IOException {
            super( s );
        }

        public NicePipedOutputStream() {
            super();
        }
        ...
        public int available() throws IOException {
            return NicePipedInputStream.FIFO_SIZE - _sink.available();
        }

        static class NicePipedInputStream extends PipedInputStream {
            public static int FIFO_SIZE = PIPE_SIZE;
        }
    }

    OutPipeHandler handler = new OutPipeHandler();

    protected class OutPipeHandler {
        private Vector<byte[]> byteArrays = new Vector<byte[]>();
        private Thread worker = null;


        public OutPipeHandler() {
            worker = new Thread( "iiVBshJConsole output handler" ) {
                public void run() {
                    try {
                        while( true ) {
                            while(byteArrays.size()>0) {
                                byte[] b = byteArrays.remove(0);
                                outPipe.write( b );
                                outPipe.flush();
                            }
